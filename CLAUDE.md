...
- **git push 前のブランチ制約チェック（必須）**: `git push` 実行前に必ず確認する
  1. システム指示・CLAUDE.md に push先ブランチの指定があるか？ → あればそのブランチへ（例: `claude/` ブランチ指定があれば `master` への push 禁止）
  2. このセッションで push が失敗したか？ → あれば同じコマンドを再実行しない
