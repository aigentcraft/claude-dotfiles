...
- **完了済みリストは直近5件のみ保持**。6件以上になったら古いものを `CHANGELOG.md` に移動すること
- **me.md の Conversation Log は直近3件のみ保持**。4件以上になったら古いものを `knowledge/conversation-archive/YYYY-MM.md` に移動すること
...
2. `~/claude-dotfiles/knowledge/error-graph/moc.md` — **エントリーポイントのみ読む**（ノード索引は全件読み込み不要。relationships.md からトラバーサルで到達する）
3. `~/claude-dotfiles/knowledge/skills-moc.md` — スキル一覧と合成プロトコル（使用時に relationships.md へ）
...
### 完了済み（直近5件 — 過去分は [CHANGELOG.md](CHANGELOG.md) を参照）
- システム監査＆10項目改善（2026-03-01）
  - MOC デ同期修正 + バリデーションスクリプト（`scripts/validate-knowledge.sh`）+ push前自動検証
  - CLAUDE.md / me.md のアーカイブ化でスケーラビリティ改善
  - MOC のトラバーサルエントリ化（通常RAG脱却）
  - フック堅牢化・session-start.sh マージ化・sync.sh 安全化
  - エスカレーション自動チェック・欠損クラスター作成・settings.json 明文化
- システムアーキテクチャ図作成（2026-02-28）
  - `SYSTEM_ARCHITECTURE.md` 新設
- Skills GraphRAG 整備（2026-02-27）
  - `relationships.md` に「システム・ガバナンスクラスター」新設
- UCスルー防止・仕組み化（2026-02-27）
  - UC検出トリガーワード + push前UCスキャン
- PAT セットアップリマインダー実装（2026-02-26）
  - 起動時リマインド + マーカーファイルで管理
