...
## 確定診断（同日3回目の実測・フォレンジックで特定）

- 3回とも**WSL側から起動した手動run（バックグラウンド）でのみ**発生。本番スケジューラ経路
  （Windows python.exe）のログには1件も出ていない = **本番は無影響**
- 症状の形: 親run_pipeline（WSL）が生存中、あるステージ以降の**新規サブプロセス接続が全滅**
  （135秒リトライも突破）。親プロセス終了と同時に即回復。残留プロセスなし・-wal/-shmも消滅
- 推定機構: DrvFs(9P)上のWAL共有メモリ/ロック変換が、WSL親の長寿命接続と子プロセスの
  新規接続の組合せで壊れる（Defender説は撤回 — 時間相関が合わない）
- 当初の「大量書き込み直後」という相関は「LLMステージは長い→その次のステージで顕在化」の
  見かけ上の相関だった可能性が高い
...
- **手動run・検証は本番と同じOS側のPython（pullieではWindows python.exe）で行う** — 検証環境が本番と違うと、検証自体が偽の障害を作る
