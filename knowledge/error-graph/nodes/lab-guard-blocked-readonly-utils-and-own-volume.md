...
- 追記（2026-08-29 記事11 Discord bot ラボ）: ヒアドキュメント（`cat > f <<'EOF' … EOF`）の本体行を「コマンド」として先頭語検査し誤ブロック → `split_segments` がヒアドキュメント本体を 1 セグメントに畳む。文字列中の「login」（`echo "… login attempt"`・`client.login()`）を正規表現で禁止していた → ログイン判定はセグメントの先頭 2〜3 語（`docker login` / `gh auth login`）だけに。**正規表現を「どこに現れても」で使う禁止規則は、必ず引用文字列・ヒアドキュメント・URL で誤検知する — 位置（先頭語・URL パス）で判定する**
- 追記（2026-08-29 記事12）: `START=$(date +%s)` の `+%s)` と、`curl \`+改行の次行 `-s …` を先頭語として誤ブロック → コマンド置換 `$(…)`/バッククォートを引数扱いに置換し、行継続を 1 行に戻してから分割。累計 5 クラスの誤ブロックはすべて「シェル構文の理解不足」— 許可リスト型ガードには最低限、引用・ヒアドキュメント・置換・継続・サブシェル・パイプの各構文テストを最初から用意する
