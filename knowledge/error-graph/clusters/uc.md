...
### R-WIRED: 設計書の手順は、呼ぶコードと「踏んだか」の検証を置くまで実装ではない
「サジェスト回帰→全語のボリューム→意図クラスタ→競合取得のうえで企画できているのか？」— できていなかった。
道具は全部あり、docs/07 §6.2 に手順も書いてあったが、企画補充のコードはどれも呼ばず、9/06 に手で 1 回回しただけの
手順が「設計」として残っていた。LLM は種の語 + 自分の知識で構成案を書いた。
- **手順書を書いたら、それを呼ぶコードと「踏んだか」を数える検証（サジェスト N・指紋 N・競合 N）を同時に置く**
- 「企画した」を報告する前に、企画が**何を材料に**作られたかを数える。登録されたことは完了の証拠ではない
- 材料が「あれば使う」設計は新しい対象に永久に材料が無い。**無ければ取りに行く**を既定にする
- 材料の実在は③物理ゲートにできる（判断は覆さない — 材料に結ばれない提案を登録しないだけ）
- 詳細: [[../nodes/uc-planned-without-the-designed-keyword-research.md]]

### R-EXIT: 「必ず通す」ルールは、通らないと動かない場所（出口）に置く
「ファイル名だけで言われても何もわからん。役割と機能で説明させて」— 4 回目（2026-09-19）。対応表と「通す」ルールは 3 回目で
**5 回目は一般的な技術用語だった**（2026-09-21「ラッパーって何のこと？」）。
一般語かどうかは基準にならない。基準は**相手がこれまでの会話でその語を使ったか**。
初出は役割で言い換え、説明を求められたら**以後どう書くか**まで決める。
作ったが、Discord 送信 27 ファイル・51 箇所のどれかが識別子をそのまま流した。
- 書く側の注意に頼らず、通知の出口（`notify.discord`）で機械的に「役割（識別子）」へ直す
- 対応表は「全スクリプトに役割名がある」検査で守る（足し忘れると赤）
- 未登録の識別子は送りつつ記録する（黙って素通しにしない）
- 詳細: [[../nodes/uc-explained-by-filename-not-by-role.md]]

### R-EXTERNAL-STATE: 進捗の判定に、外部サービスや利用者が書き換える状態を使わない
「未読」を新着の判定に使い、**利用者が自分のアプリで先に開いた連絡を永久に埋もれさせた**。
ログは「30 件を確認・新着なし」で、例外も警告も出ない。
- 処理済みは**自分の DB**で持つ。外部の状態は初期値の参考までに留める
- 使わざるを得ないなら、時刻や ID などこちらで観測できる手がかりを併用する
- **「N 件を確認 / 0 件処理」が続いたら異常を疑う**（原則 8 の再確認）
- 利用者の「本当に動いてる？」には**実測で答える**。ログの読み上げは答えにならない
- 詳細: [[../nodes/uc-unread-flag-buries-messages-the-user-already-read.md]]

### R-WHERE: 「あなたのやること」は、その通知が届く場所で完結させる
「ディスコードでの承認通知の依頼内容が、ディスコード上での承認ではなく、クロードコードで承認するような依頼になっている」
（2026-09-21）。返信での承認は半年前から実装済みだったのに、依頼文は別のアプリを指していた。
- 通知の文面に別アプリを開かせる手順を書かない。押せる UI（ボタン）で完結させる
- 作った経路は依頼文からも到達できるようにする（実装と案内はセットで直す）
- 押せる UI を出す前に「押したら効くか」を機械で確かめ、確かめられないなら出さない
- **これは 2 回目**。同じ指摘と同じ解決策（ボタン + 理由モーダル → 受け口 → D1 → 回収）が
  2026-08-07 の [[../nodes/uc-approval-flow-not-operable-from-notification.md]] に既にあった。
  新しいプロジェクトで同じ部位を作る時は、着手前に error-graph をその部位名（承認 / 通知 / 公開 / 課金）で引く。
  移植で運ばれるのはコードだけで、UX の完成条件という決定は運ばれない
- 詳細: [[../nodes/uc-approval-asked-in-the-wrong-place.md]]

### R-RECOVERY: N 個目を足したら、正常系ではなく**回復系**の経路を数える
「想い結びのスレッドでママワークスの企業とのメッセージページが取得できなかったと通知が来ているがなんで？」（2026-09-21）。
2 サイト目を足した時、受信→通知→送信は全部多サイト対応したが、**やり直し（非承認）だけが 1 サイト目固定**だった。
- 回復系（やり直し・救出・置き去り回収・再送）は普段走らないので、動作確認で踏まない
- 失敗が例外でなく**丁寧な文面**で出ると、「対応していない」ではなく「調子が悪い」に見える
- 利用者向けの文面にサイト名など固有名詞を直書きしない。レジストリから引く
- 詳細: [[../nodes/uc-recovery-path-hardcoded-to-the-first-site.md]]

### R-KEY: 不可逆操作の対象は「入れ物」ではなく「押された操作そのもの」から決める
承認ボタンの対象を Discord スレッド ID で逆引きしていた。1 スレッド = 1 会話のうちは正しいが、
**同じ企業の会話を 1 投稿に集約した瞬間に、承認したのと違う下書きが違う経路で送られうる**。
- 入れ物（スレッド・チャンネル・ページ）の ID は、中身が 1 つのうちだけ鍵として機能する
- 集約・統合を入れる予定があるなら、**集約より前に鍵を直す**
- 一意に決まらない時は「たぶんこれ」で実行せず、実行しなかったことを人に伝える
- 手順書の順序も検算する。「4 を最後に」と書いてあっても、4 の前提が 1〜3 で揃うとは限らない
- 詳細: [[../nodes/approval-keyed-by-container-breaks-on-aggregation.md]]

### R-ACT: 通知は「読んだ人が次に何をするか」から書く（状態の羅列から始めない）
「成果の不変条件が3件成立とかいう通知がディスコードに来てる…直感的にわかりずらい」（2026-09-21）。
題名は実際には「3 件**不成立**」で、1 文字違いの否定語が読み違えられていた。
- 内部語（不変条件・不成立）を人に送らない。肯定形と 1 文字違いの否定語は特に危ない
- **誰が直すのか**を検査自身に持たせ、題名で言い切る（あなた / コード側 / 自動で直る）
- 0 件の事実・UTC の時刻・識別子を人が読む行に出さない。経過は整数の日か時間で
- 抑止の鍵に毎回変わる数字を入れない（値で比べると 25 回中 25 回送ってしまっていた）
- 同じ型の指摘は 2026-09-06 にも受けている（[[../nodes/uc-mechanical-notifications-lack-situation.md]]）。
  新しい通知を足すときは、既にある良い型に寄せる
- 詳細: [[../nodes/uc-alert-in-internal-words-with-no-next-step.md]]

### R-WHY: エラー文には「症状」ではなく「原因」と「次の行動」を書く
「求人 URL が記録されていないため応募できません。**なぜ記録されていないのでしょうか？**」（2026-09-21）。
実際は元データに URL が無く、取り込みは正常だった。文面が実装都合の言い方をしたために、
利用者は**存在しないバグを探しに行った**。
- 「記録されていない」「取得できませんでした」だけで終えない。なぜそうなるのかを書く
- 代わりに何をすればよいか（この例では「スレッドで返信」）まで書く
- **失敗する操作はそもそも押せないようにする。** 文面を丁寧にするより効く
- **ただし「押せなくする」と「厚く説明する」は足し算ではない。** 同じ日に続報:
  「〜してください。という表示が出て不便です」。導線を消したら説明は 1 行まで削る。
  解決済みの問題への釈明を毎回出すと、恒久的な雑音になる
- **「押せなくした」と言う前に、その操作への導線を数える。** カードのボタンだけ塞ぎ、
  スレッドの応募文に付く同じ操作 ID は生きていた（repair が作り続けていた）。
  操作 ID の生成箇所を grep で全部出し、全部に同じ条件を置いたかを確かめる
- 画面を占有する文言は**長さ自体を検査で縛る**（1 行・N 文字以下は純関数で測れる）
- 切り分けは**由来別・経路別の集計**から。全体の不具合か経路固有かが一目で分かる
- 詳細: [[../nodes/uc-error-message-names-the-symptom-not-the-cause.md]]

### R-DEAD-END: 代替行動を案内する文言は、その行き先が動くか確かめてから書く
「スレッドで返信してください、というけど、**スレッドへのリンクが無くて不親切**なんだよね」（2026-09-21）。
調べると二重に悪く、**その Discord スレッドに書いても企業には届かなかった**
（受け取った発言をログに出すだけで、送信経路が無い）。返信先の URL は取れていたのに使っていなかった。
- 「〜で返信してください」と書いた瞬間に、**その経路の実装を辿る**。辿らずに書いた案内は行き止まり
- 場所を指したら**リンクを必ず添える**。直接開けない時は「一覧を開く」と**着地点を正直に書く**
  （「返信する」と書いて 41 件の一覧に落とすのは、リンクが無いのと同じ不親切）
- **文面を短くする作業は、文面が正しいかを検査しない。** 推敲と検証は別の作業
- 詳細: [[../nodes/uc-told-user-to-act-somewhere-that-does-not-work.md]]

### R-FRAME: 同じ箇所を 3 回直しているなら、問題は文面ではなく枠組みにある
「応募先の求人がない、ではなく、**やり取りへの導線を案内して**ほしい」（2026-09-21・同じ 1 行で 4 度目）。
説明を厚く → 1 行に削る → リンクを付ける、と 3 回直したが、すべて
**「求人ページがありません」という枠の中**での改良だった。何を主語にするかを一度も疑わなかった。
- **2 回目の指摘が来た時点で、その文が何を主語にしているかを疑う**
- **機械から見た事実と、利用者から見た事実は別物。** 「求人 URL が無い」は実装の観測。
  「企業と 37 通やり取りしていて返事待ちが 2 日」が相手の現実。人に見せる面には後者を書く
- **「LLM に読ませれば分かるはず」と言われたら、まず記録を見る。** 通数・最後の話者・経過は
  数えれば出る。推論の要らない判断にトークンを払わない（毎回全件描き直す面では特に）
- 詳細: [[../nodes/uc-error-message-names-the-symptom-not-the-cause.md]] §6

### R-DEATH: 常駐は死因を書いてから死ぬ／起動用リスナを残さない
「承認して応募する、のボタンをおしたけど時間内に反応しなかった」（2026-09-21）。
常駐が終了コード 1 で停止していたが、**ログに理由が 1 行も無かった**。
- `once(Events.Error, reject)` を接続後も残すと、1 回目で外れ **2 回目は listener 不在で throw**。
  起動待ちのリスナは接続できた時点で外し、恒久リスナに差し替える
- `unhandledRejection` / `uncaughtException` を登録し、スタックを書いてから終了する
- 利用者に見える症状は「操作が効かない」だけ。**死因が無いと調査手段がゼロになる**
- 詳細: [[../nodes/once-error-listener-lets-the-daemon-die-silently.md]]

### R-SCOPE: 1 つ分のコンテキストを取る処理に、全体のデータを流し込まない
整合コマンドが案件をサイトで絞らずに回し、**他サイトの 40 件を別サイトのボードに作った**（2026-09-21）。
- テナント・サイト単位の資源（チャンネル・プロファイル・認証）を引数に取る処理は、
  **入口でデータも同じ単位に絞るのが唯一の防御**。レコードの型は同じなので型検査は助けない
- **整合コマンドは必ずドライランしてから実行する。** 件数が桁違いならそこで止まれる
- 詳細: [[../nodes/repair-command-fed-all-sites-into-one-board.md]]

### R-SIBLING: 「動いていない／待ち」と報告する前に、隣のプロジェクトとナレッジを見る
「weeveeの方だと、検索ボリュームの取得が申請ではなく別経路になってたと見つけてくれて実際に取得できているよ」（2026-09-23 pullie）。
「Google Ads の審査待ち」と報告したが、同じ罠を weevee が 9 日前に抜け、共有ナレッジに予防ルールまで書いてあった。
- **記録の「待ち」は書かれた日の事実。** 報告の前に今日のエラーコードを読む
- **状態を報告する前に共有ナレッジを依存先名で grep する**（エラー時だけでなく「動いてる？」と聞かれた時も）
- **同じユーザーの隣のプロジェクトは参照先**（weevee ⇄ pullie は相互に移植元）
- 詳細: [[../nodes/uc-reported-stale-wait-while-sibling-had-the-fix.md]]

### R-AUDIENCE: 「誰のどんな反応を狙う記事か」を、企画・執筆・検閲の 3 か所で同じ文面にする
「読者が『すごい！』『なるほど！』『使ってみたい！』と思えるような記事にしないとダメ。いろんな分野のAIによる業務の自動化や
効率化の情報収集をXも含めて積極的に行い、誰よりも早く」（2026-09-23 weevee）。
速報記事の指示は 3 担当とも「乗り換え判断」の視点だけで、驚き・理解・試したさはどこにも無かった。
集める範囲も、担当に任せた計画が既存製品の語で埋まり、業務の分野は 0 だった。
- **狙いは単一ソースにし、3 担当が同じ文面を読む**（1 か所だけに書くと、他の担当は別の基準で書いて採点する）
- **集める範囲はメディアの目的から見た「まだ見ていない分野」を明示し続ける**（任せると既存の周りに縮む）
- **計画した検索が何件集まったかを数える**
- 詳細: [[../nodes/uc-news-must-show-what-ai-can-now-do-across-fields.md]]

### R-NOT-A-MANDATE: 禁止のルールを、義務のルールに読み替えて機械化しない
「推論の実測はしなくていいよ、公式に情報あるでしょ。…実測が必要な内容であっても公式などに載っていればわざわざやる必要なし」（2026-09-23 weevee）。
企画の検査が「outline に『実測』が無ければ落とす」で、根拠に「実測にない体験談を書かない（絶対ルール 9）」を挙げていた。
ルール 9 は捏造の禁止であって、実測の義務ではない。結果、公式に情報がある推論性能まで実測する企画ができた。
- **ゲートの根拠に規則を引くときは、規則の文面が本当にそれを求めているかを読み直す**
- **一次情報がある事実は測り直さない**（実測は公式で答えられない問いに使う時間）
- 詳細: [[../nodes/uc-do-not-measure-what-is-officially-published.md]]


### 2026-09-23: 公開面に「どう調べたか・何で止まったか」を書かない
「Bot確認画面で止まったなんて書いたら自動化して書いてるのもろばれ」。8/29 の内部語の検出（MC05）は語の列挙で、
「ボット確認画面」「到達できず」「ボタンの先は未確認」を素通りした。原則を 1 か所に書き執筆・検閲に同じ文面で渡す。
- 詳細: [[../nodes/uc-bot-check-wall-written-into-public-article.md]]

### 2026-09-23（夜）: 「公開面」は本文ではなく、画面に出る文字列の全部
同日の「調べる手段を公開面に書かない」の検査（MC05）は frontmatter を最初から読み飛ばしていた。見出し欄の「環境」に
「公式サイト閲覧のみ（Playwrightヘッドレス・フルページ撮影）」「未ログイン…確認のみ」が本番で 5 本、どれも「実測日」つき。
- **公開面の検査対象は、レイアウトが表示する文字列（frontmatter・キャプション・alt・OG）をコードから数えて決める**
- **書き手に届けたい決まりをコードのコメントに書かない**（「実測が無ければ省略してよい」はコメントにしか無かった）
- 詳細: [[../nodes/public-surface-check-skipped-rendered-frontmatter.md]]
