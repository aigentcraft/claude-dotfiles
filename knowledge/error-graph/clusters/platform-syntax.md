...
- **ヘッダの折り返し（次の行が空白始まり）は、読む関数の中でつなぐ。** 呼び出し側の前処理に頼ると、
  経路が増えた時に片方だけ件名が空になる（2026-09-26 クラウドワークス）。検査には長い件名も入れる
- 詳細: [[../nodes/mime-7bit-body-is-already-decoded.md]] / [[../nodes/mail-header-folded-value-read-as-empty.md]]
