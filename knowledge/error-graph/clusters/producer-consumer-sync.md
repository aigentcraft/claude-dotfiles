...
### R6: 発見側が作る集合と、消費側が読む集合を、別の状態にしたまま繋がない
生産側が新しく見つけたものを「未検証」の状態（`candidate` / `draft` / `pending`）で置き、
消費側が「検証済み」の状態だけを読むと、**自分の発見を自分で見えなくする閉路**ができる。
昇格経路が「人／別処理がたまたま使ったとき」しか無いと、生産は回り続けるのに供給はゼロ。
- 例: weevee のキーワード調査は、サジェストで見つけた語を `candidate` で登録し、
  種を選ぶ側は `status='active'` だけを読んでいた。昇格は「企画に使われたとき」の 1 本のみ。
  結果、1,300/月・1,000/月 を含む **100+/月 が 20 語** 眠る一方、種は 0-9/月 の谷
  （active 66 語中 46 語）を毎週掘り、そこから出る新語は当然すべて「データなし」。
  4 段階は全部動いており rc=0・テスト緑だった（[[../nodes/seed-pool-filter-starves-the-demand-it-discovered.md]]）
- **新しい状態を足すコミットで、そこから出る自動の経路も書く**（手動昇格だけにしない）
- 絞り込み（`WHERE status=...`）を書いたら、**弾かれた側の分布を 1 回数える**。
  内側だけ見ていると、痩せていることは永久に分からない
- 供給は消費側の**唯一の入口**に置く（ここでは `pick_seeds` の先頭）。
  呼び出し側ごとに足すと、足し忘れた経路だけが古いまま動く

| 状況 | 適用するルール |
|---|---|
| 発見したものを「未検証」状態で置く | R6: 昇格の自動経路を同じコミットで。弾かれた側の分布を数える |
