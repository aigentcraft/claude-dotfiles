...
### R21: `py_compile` が通っても import できるとは限らない — 必ず import して確かめる
`py_compile` は**構文しか見ない**。未定義の名前を関数シグネチャの型注釈に書いても通る。
実例: `def f(path: Path)` と書いて `Path` を import し忘れ、`py_compile` は成功したが
モジュールの import 時に `NameError` で落ちる状態だった（本番なら画像生成レーンが丸ごと停止）。
- 検証は `py_compile` でなく **`importlib` で実際に読み込む**（副作用が怖いなら
  `main()` を呼ばずモジュールの読み込みだけ行う）
- 型注釈は実行時に評価される（`from __future__ import annotations` が無い限り）

### R20: 「推奨」は制御ではない — 守らせたい規約は機械で塞ぐまでが対策
コメントやスキル文書に「〜を推奨」と書いても守られない。実例: 撮影キーに
「アプリ指定を推奨」とコメントが入ってから2週間以上たっても、**公開記事の題材依存
スクショ参照の28%が無指定のまま**で、既定アプリ（自社の業務アプリ）が公開記事に写っていた。
- 規約を守らせたいなら、**守らなかった時に止まる分岐**を入れる（既定値で続行しない）
- 止めるなら**理由を記録して下流に届ける**（黙って飛ばすと、今度は欠落が見えなくなる）
- 対象の判定は**一覧を二重管理せず、データから機械的に導く**
  （例: 「撮影URLに対象アプリを含むキーか」をカタログから判定する）
- 詳細: [[../nodes/capture-success-is-not-content-correctness.md]]

### R19: 「やってよい」と書くだけでは実行されない — 境界のある単位にして必ず答えさせる
無限界の仕事（「全部を見て必要なら直して」）を渡すと、エージェントは**境界のある部分だけ**を
やって完了と見なす。実行してほしい判断は「今回はこのN件」と区切り、**全件に verdict と理由を
返させる**。どれを見せるかの選択は機械（点検が古い順など・内容では選ばない）、
keep/demote/retire の判断はエージェント。この線を越えると R18 の判断ハードコードになる。
- 能力の有無ではなく**行使の有無を実測**する（316件中 retired 1件のような偏りは仕組みの症状）
- 材料から**由来を落とさない**。記録するだけでは足りず、使う側に届くまでが経路
- 詳細: [[../nodes/unbounded-review-task-collapses-to-bounded-part.md]]
...
壊れた瞬間から誰も気づけない。**どちらも rc=0 で正常に見える**。
- 計器を作ったら「**正しい成果物**で 1 回通す」（壊れた成果物で赤が出るだけでは半分）
- 役ごとの計器一覧を単一ソースにし、「その役の計器すべてが契約を満たした成果物で緑になる」を毎回検査する
- 「検査できなかった」を「合格」と同じ値にしない。法令系は fail-closed
- 工程で意味が変わる述語に、どちらとも読める名前を付けない
- 詳細: [[../nodes/instrument-the-agent-cannot-turn-green.md]]
