...
- [[nodes/approval-keyed-by-container-breaks-on-aggregation.md]] — `approval-flow` cluster (`irreversible-action`, `identity`, `aggregation`, `discord`)
- [[nodes/arrival-notice-hardwired-to-first-site-channel.md]] — `discord` cluster (`routing`, `multi-site`, `default-destination`, `notification`, `fukugyo-hootl`)
...
- [[nodes/detached-launch-announces-start-never-reports-outcome.md]] — `observability` cluster (`detached-process`, `pipeline`, `silent-degradation`, `weevee`)
...
- [[nodes/git-bash-msys-path-conversion-mangles-slash-args.md]] — `git-bash` cluster (`msys`, `path-conversion`, `cmdkey`, `windows`, `fukugyo-hootl`)
...
- [[nodes/guard-and-its-check-must-not-share-the-same-handle.md]] — `privacy` cluster (`masking`, `verification`, `selector-drift`, `fail-open`, `weevee`)
- [[nodes/guard-on-my-output-leaked-into-agent-output.md]] — `hooks` cluster (`self-inflicted`, `pullie`, `blast-radius`, `silent-failure`)
...
- [[nodes/headless-ua-bot-detection.md]] — `playwright` cluster (`bot-detection`, `cloudflare`, `headless`, `scraping`, `hootl`)
...
- [[nodes/kintone-form-editor-rebuilt-old-selectors-gone.md]] — `kintone` cluster (`playwright`, `selector`, `screenshot`, `external-breakage`, `pullie`)
...
- [[nodes/placeholder-example-domain-executed-verbatim.md]] — `credentials` cluster (`placeholder`, `guard`, `cli-guidance`, `human-in-the-loop`, `fukugyo-hootl`)
...
- [[nodes/planner-promised-trial-the-lab-cannot-run.md]] — `planning` cluster (`measurement`, `notification`, `observability`, `missing-material`, `weevee`)
...
- [[nodes/powershell51-scalar-count-is-null.md]] — `powershell` cluster (`windows`, `version-difference`, `reporting`, `fukugyo-hootl`)
...
- [[nodes/public-surface-check-skipped-rendered-frontmatter.md]] — `editorial` cluster (`internal-leak`, `automation-exposure`, `compliance`, `frontmatter`, `incomplete-fix`, `weevee`)
- [[nodes/publish-commit-sweeps-foreign-staged-changes.md]] — `git` cluster (`pipeline`, `publish`, `security`, `idempotency`, `weevee`, `pullie`)
...
- [[nodes/regenerated-images-never-reached-live-site.md]] — `pipeline-idempotency` cluster (`silent-failure`, `deploy`, `image-generation`, `usage-limit`, `observability`, `weevee`)
...
- [[nodes/reviewer-says-unfixable-but-loop-keeps-retrying.md]] — `review-loop` cluster (`agent-contract`, `pullie`, `waste`, `silent-failure`)
...
- [[nodes/same-counterpart-two-channels-two-truths.md]] — `multi-channel` cluster (`identity`, `state-conflict`, `context-loss`, `discord`, `fukugyo-hootl`)
- [[nodes/same-set-defined-in-four-places-one-silently-strips.md]] — `single-source` cluster (`internal-links`, `silent-strip`, `gate-wording`, `weevee`)
- [[nodes/screenshot-edit-area-border-gate-impossible-for-wide-ui.md]] — `rendering-quality` cluster (`impossible-condition`, `image-gate`, `screenshot`, `weevee`)
...
- [[nodes/stop-script-killed-every-playwright-browser.md]] — `windows` cluster (`powershell`, `process-tree`, `playwright`, `orphan`, `kill-scope`, `fukugyo-hootl`)
- [[nodes/stop-task-leaves-either-wrapper-or-child.md]] — `windows` cluster (`task-scheduler`, `powershell`, `process-tree`, `orphan`, `fukugyo-hootl`)
...
- [[nodes/thread-message-logged-only-never-answered.md]] — `human-in-the-loop` cluster (`discord`, `thread`, `silent`, `rule-not-enforced`, `fukugyo-hootl`)
...
- [[nodes/win32-shell-spawn-args-parsed-by-cmd-exe.md]] — `windows` cluster (`node`, `child_process`, `cmd.exe`, `shell`, `taskkill`, `npm-shim`, `fukugyo-hootl`)
...
- [[nodes/writer-internal-handoff-notes-leak.md]] — `writer` cluster (`public-tone`, `mechanical-strip`, `llm-behavior`, `weevee`)
...
- [[nodes/writer-vocabulary-crashes-demo-app-build.md]] — `kintone` cluster (`dsl`, `llm-output`, `producer-consumer-sync`, `pullie`, `screenshot`)
...
- [[nodes/uc-approval-requested-with-non-gpt-images.md]] — `pipeline-idempotency` cluster (`uc`, `fallback`, `image-generation`, `grace`, `human-in-the-loop`, `discord`, `weevee`)
...
- [[nodes/uc-log-watch-matched-only-some-outcomes.md]] — `uc` cluster (`ai-behavior`, `monitoring`, `silent-failure`, `log-watch`, `fukugyo-hootl`)
- [[nodes/uc-mechanical-automation-wastes-llm-metacognition.md]] — `uc` cluster (`pullie`, `architecture`, `metacognition`, `agent-design`, `llm`)
- [[nodes/uc-mechanical-notifications-lack-situation.md]] — `uc` cluster (`pullie`, `notification`, `reporting`, `human-in-the-loop`, `observability`)
- [[nodes/uc-model-comparison-too-shallow-competitor-bypass.md]] — `uc` cluster (`editorial`, `comparison`, `competitor`, `research-protocol`, `sns`, `weevee`)
- [[nodes/uc-news-must-show-what-ai-can-now-do-across-fields.md]] — `uc` cluster (`editorial`, `x-collection`, `news`, `weevee`)
- [[nodes/uc-notification-instead-of-self-healing.md]] — `uc` cluster (`pullie`, `notification-policy`, `self-healing`, `hootl`)
- [[nodes/uc-notification-must-carry-shared-entity-ids.md]] — `pullie` cluster (`notification-design`, `shared-identifiers`, `hitl`, `approval-flow`)
...
- [[nodes/uc-prevention-rule-written-but-not-enforced-in-code.md]] — `uc` cluster (`ai-behavior`, `learning-loop`, `prevention-not-enforced`, `polling`, `cloudflare`, `human-in-the-loop`, `fukugyo-hootl`)
...
- [[nodes/uc-read-the-brand-when-user-meant-their-own-instance.md]] — `uc` cluster (`ai-behavior`, `intent`, `cloudflare`, `browser-automation`, `fukugyo-hootl`)
...
- [[nodes/uc-silent-fallback-labeled-as-gpt.md]] — `uc` cluster (`images`, `imagegen`, `codex`, `fallback`, `silent-failure`, `labeling`, `self-healing`, `observability`, `weevee`)
